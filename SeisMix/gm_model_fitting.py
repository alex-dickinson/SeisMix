...
def gm75_residual_power(vars, x, y, sigma_y):
    ...
    gm75_model = (
        np.log10(C)
        - np.log10(kstar)
        - (t / s) * np.log10(1.0 + np.power((kx / kstar), s))
    )
...
def fit_gm75_kx_power_parametrisation_leastsq(logkx, logphi, sigma_logphi, par):
    ...
    kx = np.power(10.0, logkx)
    gm75_model_initial = (
        np.log10(C_initial)
        - np.log10(kstar_initial)
        - (t_initial / s_initial)
        * np.log10(1.0 + np.power((kx / kstar_initial), s_initial))
    )
    ...
    best_par = optimize.leastsq(
        gm75_residual_power, par, args=(logkx, logphi, sigma_logphi)
    )
    ...
    gm75_model_fitted = (
        np.log10(C_fitted)
        - np.log10(kstar_fitted)
        - (t_fitted / s_fitted)
        * np.log10(1.0 + np.power((kx / kstar_fitted), s_fitted))
    )
