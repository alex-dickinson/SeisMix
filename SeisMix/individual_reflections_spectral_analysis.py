...
def calculate_spectra(midpoints, sound_speed_f, dx, jres_mtft_f, K_mtft_f):
    ...
    power_spectrum, kx, jackknife, _, _ = mtspec(data=depths, delta=float(dx), time_bandwidth=jres_mtft_f, number_of_tapers=K_mtft_f, nfft=None, statistics=True) 
#     r = pymutt.mtft(depths, kind=ki, npi=jres_mtft, nwin=K_mtft, dt=dx)
#     power_spectrum = r['power']
#     kx = np.arange(r['nspec']) * r['df']
    slope_spectrum = power_spectrum * (np.power((2.*np.pi*kx), 2.))
...
def estimate_sigma_logphi(spectrum_confidence_limit_low, spectrum_confidence_limit_high, sound_speed_sigma_sound_speed):
    sigma_logphi = np.power(((np.power(sigma_logamp, 2.) + np.power((2.*np.log10(np.exp(1.))*(sigma_sound_speed/sound_speed)), 2.))), 0.5)
    return(sigma_logphi)
...
def inst_model(logkx,par):
    ...
    turb = 1./3. * (logkx-par[0]) + par[1]
    noise = 2. * (logkx-(par[0]+par[2])) + (par[1] + 1./3. * par[2])
...
def matts_calc(par, gamma, CT, N_rads):
    top=pow(10.,par[1])*N_rads*N_rads
    bottom=4.*np.pi*gamma*CT*(pow(10,par[0])*2*np.pi)**0.333333333
...
def compute_potential_energy(variance, N_rads, rho_kg_m3):
    gpe = 0.5 * rho_kg_m3 * np.power(N_rads, 2.) * variance
    return(gpe)
...
def iw_slope_uncertainty(logkx, logphi):
    ...
    residual_squared = np.power((logphi[:] - best_model_iw[:]), 2.)
