...
def fit_intercept_with_fixed_gradient(xdata, ydata, sigma_y, m, x0):
    ...
    S = np.sum(1.0 / np.power(sigma_y, 2.0))
    Sx = np.sum(xdata / np.power(sigma_y, 2.0))
    Sy = np.sum(ydata / np.power(sigma_y, 2.0))

    best_c = (Sy - m * Sx) / S
    sigma_c = np.power((1.0 / S), 0.5)
...
def estimate_epsilon_from_turbulent_subrange(
    logkx, logphi, sigma_logphi, N_rads, sigma_N_rads, gamma, CT
):
    ### Find best-fitting intercept for fixed gradient, m, of 1/3. x0 is initial guess of intercept.

    m = 1.0 / 3.0
    x0 = 0.0
    c2_turb, sigma_c2_turb = fit_intercept_with_fixed_gradient(
        logkx, logphi, sigma_logphi, m, x0
    )
    ...
    dof = np.size(logkx) - 1
    chi_sq_data_turb = chi_square_analysis.chi_squared(
        logkx, logphi, m, c2_turb, sigma_logphi
    )
    ...
    a_well, b_well, d_well = chi_square_analysis.chi_squared_well_direct_expression(
        logkx, logphi, m, c2_turb, sigma_logphi
    )
    ...
    n = 2.0
    plus_c2, minus_c2 = chi_square_analysis.chi_squared_acceptance_levels(
        logkx, logphi, m, c2_turb, sigma_logphi, chi_sq_data_turb, n
    )
    ...
    sigma_logepsilon, sigma_logkt = epsilon_kt_error_propagation(
        c2_turb, sigma_c2_turb, N_rads, sigma_N_rads
    )
