...
def estimate_mixing(avespec_array, N_cph, sigma_N_cph, latitude_degrees, gamma, CT, gm_option, N0_cph, N_GM_cph, E, b, jstar, RwGM, Rw, latitude0, Rf, Nkz, logkz_gm_min, logkz_gm_max, Nkx, logkx_gm_min, logkx_gm_max, NkH, gm_rolloff_option, min_turb_length, min_iw_length, averaging_option):
    ...
    par = [-2, -3, 0.5, -1.] # Initial guess of model parameters
    fitted_pars = four_component_model_subrange_fitting.subrange_fitting(logkx, logphi, par)
...
#     plt.plot(turb_logkx, c2_turb + 1./3.*turb_logkx)
    ...
    if np.size(turb_logkx) >= min_turb_length:
        ...
        m1_turb_fit, sigma_m1_turb_fit, c1_turb_fit, sigma_c1_turb_fit = straight_line_fitting.fit_straight_line(turb_logkx, turb_logphi, turb_logphi_std, x0)
...
#         print(sigma_m1_turb_fit, "sigma_m1_turb_fit")
    ...
    #	if m1_turb_fit - sigma_m1_turb_fit <= 0.33 and m1_turb_fit + sigma_m1_turb_fit >= 0.33 and sigma_m1_turb_fit <= 0.33:
        if m1_turb_fit - 100. <= 1./3. and m1_turb_fit + 100. >= 1./3.:
#             print "turb"

            ### Estimate epsilon from turbulent subrange by fitting straight line of gradient 1/3 ### TODO Check straight line fitting
            c2_turb, sigma_c2_turb, logepsilon_turb, sigma_logepsilon_turb, logK_turb, sigma_logK_turb = estimate_turbulent_epsilon.estimate_epsilon_from_turbulent_subrange(turb_logkx, turb_logphi, turb_logphi_std, N_rads, sigma_N_rads, gamma, CT)

    #		plt.plot(logkx, logphi)
    #		plt.plot(turb_logkx, turb_logphi)
            plt.plot(turb_logkx, c2_turb + 1./3.*turb_logkx)
    #		plt.show()
            if m1_turb_fit - 0.3 <= 1./3. and m1_turb_fit + 0.3 >= 1./3.:
...
#             print logK_turb, sigma_logK_turb, "logK_turb", "sigma_logK_turb"
    ...
    #	print np.size(iw_logkx)
        ...
        ### ### TODO x0 can be defined in module
        x0 = np.array([0.0, 1.0])
        m1_iw_fit, sigma_m1_iw_fit, c1_iw_fit, sigma_c1_iw_fit = straight_line_fitting.fit_straight_line(iw_logkx, iw_logphi, iw_logphi_std, x0)
    ...
    #	print sigma_c1_iw_fit, "sigma_c1_iw_fit"
        ...
        gm_fit_initial = np.array([100., 1., 2.5, 0.0001])
        A_gm_fit, s_gm_fit, t_gm_fit, kstar_gm_fit = gm_model_fitting.fit_gm75_kx_parametrisation_leastsq(iw_logkx, iw_logphi, iw_logphi_std, gm_fit_initial)
        ...
        if gm_option == "gm76":
            logkx_reference, power_spectrum_reference, logphi_reference = compute_gm76_horizontal_spectra.run_kx_from_kxkz(N_rads, N0_rads, f, Nkz, logkz_gm_min, logkz_gm_max, Nkx, logkx_gm_min, logkx_gm_max, NkH, E, b, jstar, gm_rolloff_option)
        elif gm_option == "gk91":
            logkx_reference, power_spectrum_reference, logphi_reference = compute_gk91_horizontal_spectra.run_kx_from_kxkz(N_rads, N0_rads, f, Nkz, logkz_gm_min, logkz_gm_max, Nkx, logkx_gm_min, logkx_gm_max, NkH, E, b, jstar, gm_rolloff_option)
        else:
            print("gm_option not specified - exiting")
            exit()

        ### Make fitted spectrum for output
        logphi_fitted_gm = gm_model_fitting.make_fitted_spectrum(logkx_reference, A_gm_fit, s_gm_fit, t_gm_fit, kstar_gm_fit)
        ...
        logepsilon0, logK0, logepsilon_iw, logK_iw, logK_error_iw, sigma_logepsilon_iw_low, sigma_logepsilon_iw_high, sigma_logK_iw_low, sigma_logK_iw_high = estimate_internal_wave_epsilon.estimate_epsilon_from_internal_wave_subrange(logkx, logphi, sigma_phi, logphi_std, iw_logkx, iw_logphi, iw_logphi_std, iw_kx_lower_limit_integration, iw_kx_upper_limit_integration, logkx_reference, logphi_reference, E, N0_rads, b, jstar, N_GM_rads, Rf, Rw, RwGM, N_rads, f, f_GM, gm_option)
