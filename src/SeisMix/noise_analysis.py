...
def make_amplitude_and_phase_spectra(tx, sampling, cmp_spacing):
    ...
    return(f, kx, amplitude_spectrum_sorted, phase_spectrum_sorted, kx_spectrum, f_spectrum)
...
def make_direct_data_transform(tx, cmp_spacing, jres, K):
    ...
    def make_level_spectra(tx, cmp_spacing, jres, K):
        ...
        for i in range(number_levels):
            level = tx[i,:]
            power_spectrum, kx, jackknife, _, _ = mtspec(data=level, delta=float(cmp_spacing), time_bandwidth=jres, number_of_tapers=K, nfft=None, statistics=True)
            if i == 0:
                power_spectra_array = np.zeros([np.size(tx[:,0]), np.size(power_spectrum)])
...
#         todo - error on DDT
    ...
    kx, ddt_power_spectrum, ddt_power_spectrum_stdev = make_level_spectra(tx, cmp_spacing, jres, K)
    ddt_slope_spectrum = np.power((2.*np.pi*kx), 2.) * ddt_power_spectrum
    ddt_slope_spectrum_stdev = np.power((2.*np.pi*kx), 2.) * ddt_power_spectrum_stdev
#     Todo: when is correct point to average error?
    
    return(kx, ddt_power_spectrum, ddt_power_spectrum_stdev, ddt_slope_spectrum, ddt_slope_spectrum_stdev)
