...
def estimate_K0(E, N0_rads, b, jstar, N_GM_rads, f_GM, Rf, gm_option):
	...
	kzstar = jstar / (2.*b) * (N_GM_rads / N0_rads)
	
	if gm_option == "gk91":
		F0 = (6.*f_GM)/(10.*np.pi) * np.power((E/N0_rads), 2.) * np.power(kzstar, 2.) * np.arccosh(N_GM_rads / f_GM) * np.power((b*N0_rads), 4.) * np.power((2.*np.pi), 2.)
		epsilon0 = (1.-Rf) * F0
		K0 = Rf * F0 / np.power(N_GM_rads, 2.)
		### TODO Check values
	elif gm_option == "gm76":
		F0 = (6.*f_GM)/(10.*np.pi) * np.power((E/N0_rads), 2.) * np.power((2./np.pi), 2.) * np.power(kzstar, 2.) * np.arccosh(N_GM_rads / f_GM) * np.power((b*N0_rads), 4.) * np.power((2.*np.pi), 2.)
		epsilon0 = (1.-Rf) * F0
		### TODO Check values
		K0 = Rf * F0 / np.power(N_GM_rads, 2.)
	else:
...
def estimate_K(Rw, RwGM, N_rads, f, N_GM_rads, f_GM, epsilon0, K0, spectrum_variance, gm_spectrum_variance):
	
	shear_strain_term = Rw * (Rw + 1.) / (6. * np.power(2., 0.5) * np.power((Rw-1.), 0.5))
	### TODO Check appropriate definitions of N.
	f_term = (f * np.arccosh(N_rads / f)) / (f_GM * np.arccosh(N_GM_rads / f_GM))
	
	epsilon = epsilon0 * np.power((N_rads / N_GM_rads), 2.) * np.power(spectrum_variance, 2.) / np.power(gm_spectrum_variance, 2.) * shear_strain_term * f_term
	K = K0 * np.power(spectrum_variance, 2.) / np.power(gm_spectrum_variance, 2.) * shear_strain_term * f_term
...
def estimate_standard_deviation_spectral_ratio(spectrum_integrand, reference_spectrum_integrand):
...
def estimate_logK_sigma(integrand_ratio_std, spectrum_integrand, reference_spectrum_integrand, spectrum_variance, reference_spectrum_variance):
	
	variance_ratio = spectrum_variance / reference_spectrum_variance
	
	logK_sigma = 2. * 0.4343 * integrand_ratio_std / variance_ratio
...
def estimate_epsilon_from_internal_wave_subrange(all_logkx, all_logphi, all_sigma_phi, all_sigma_logphi, iw_logkx, iw_logphi, iw_sigma_logphi, iw_kx_lower_limit_integration, iw_kx_upper_limit_integration, logkx_reference, logphi_reference, E, N0_rads, b, jstar, N_GM_rads, Rf, Rw, RwGM, N_rads, f, f_GM, gm_option):
	
	all_kx = np.power(10., all_logkx)
	all_phi = np.power(10., all_logphi)
	all_sigma_phi_from_logphi = np.power(10., all_sigma_logphi)
	iw_kx = np.power(10., iw_logkx)
	iw_kx_min = iw_kx[0]
	iw_kx_max = iw_kx[-1]
	iw_phi = np.power(10., iw_logphi)
	...
	all_phi_from_logphi_low = np.power(10., all_logphi_low)
	all_phi_from_logphi_high = np.power(10., all_logphi_high)
...
#	print all_phi_high
#	
	all_phi_variance = np.power(all_sigma_phi, 2.)
...
#	plt.show()
	...
	iw_kx_integrand, iw_spectrum_integrand, iw_spectrum_variance = estimate_spectral_variance(all_kx, all_phi, iw_kx_lower_limit_integration, iw_kx_max)
...
#	iw_kx_integrand_high, iw_spectrum_integrand_high, iw_spectrum_variance_high = estimate_spectral_variance(all_kx, all_phi_high, iw_kx_lower_limit_integration, iw_kx_max)
	iw_kx_integrand_low, iw_spectrum_integrand_low, iw_spectrum_variance_low = estimate_spectral_variance(all_kx, all_phi_from_logphi_low, iw_kx_lower_limit_integration, iw_kx_max)
	iw_kx_integrand_high, iw_spectrum_integrand_high, iw_spectrum_variance_high = estimate_spectral_variance(all_kx, all_phi_from_logphi_high, iw_kx_lower_limit_integration, iw_kx_max)
...
#	print iw_kx_integrand
	...
	iw_variance_variance = np.sum(all_phi_variance[kx_min_arg:kx_max_arg]) * np.power( (iw_kx_integrand[1] - iw_kx_integrand[0]), 2.)
...
#	exit()
	...
	reference_spectrum_interp = np.power(10., logphi_interp)
	
	gm_kx_integrand, gm_spectrum_integrand, gm_spectrum_variance = estimate_spectral_variance(all_kx, reference_spectrum_interp, iw_kx_lower_limit_integration, iw_kx_max)
	...
	spectral_variance_ratio, epsilon, K = estimate_K(Rw, RwGM, N_rads, f, N_GM_rads, f_GM, epsilon0, K0, iw_spectrum_variance, gm_spectrum_variance)
	logK = np.log10(K)
	logepsilon = np.log10(epsilon)
	
	### Estimate error on logK
	spectral_ratio_std = estimate_standard_deviation_spectral_ratio(iw_spectrum_integrand, gm_spectrum_integrand)
	### TODO Estimate this properly
	sigma_iw_logK = estimate_logK_sigma(spectral_ratio_std, iw_spectrum_integrand, gm_spectrum_integrand, iw_spectrum_variance, gm_spectrum_variance)
	
	### Estimate error on logK using standard deviation of average Welch spectrum
	spectral_variance_ratio_low, epsilon_low, K_low = estimate_K(Rw, RwGM, N_rads, f, N_GM_rads, f_GM, epsilon0, K0, iw_spectrum_variance_low, gm_spectrum_variance)
	spectral_variance_ratio_high, epsilon_high, K_high = estimate_K(Rw, RwGM, N_rads, f, N_GM_rads, f_GM, epsilon0, K0, iw_spectrum_variance_high, gm_spectrum_variance)
...
#	print np.log10(np.exp(1.))
	logK_variance = 2.*0.4343 / iw_spectrum_variance * iw_variance_error
...
#	print logK_error, "logK_error"
	...
	return(logepsilon0, logK0, logepsilon, logK, logK_error, sigma_logepsilon_low, sigma_logepsilon_high, sigma_logK_low, sigma_logK_high)
